...
export default function Pages() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try { setRows((await base44.entities.Page.list("-created_date", 200)) ?? []); } catch { setRows([]); }
    setLoading(false);
  };
  useEffect(() => { fetchList(); }, []);
  ...
  const save = async () => {
    if (editing.id) await base44.entities.Page.update(editing.id, editing);
    else await base44.entities.Page.create(editing);
    setEditing(null); fetchList();
  };
  const remove = async (id) => { await base44.entities.Page.delete(id); setRows((p) => p.filter((x) => x.id !== id)); setDeleteConfirm(null); };

  return (
    <AdminLayout title="Pages" breadcrumbs={[{ label: "Admin", href: "/admin" }, { label: "Pages" }]}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div><h2 className="text-2xl font-bold text-white">Pages</h2><p className="mt-1 text-sm text-admuted">{rows.length} pages</p></div>
        <AdminButton onClick={() => setEditing({ ...blank })}><Plus className="h-4 w-4" /> New Page</AdminButton>
      </div>
      ...
      <Card className="overflow-x-auto p-0">
        {loading ? (
          ...
          <div className="p-5"><LegacyEmptyState icon={FileText} title="No pages yet" body="Create your first page." action={<AdminButton onClick={() => setEditing({ ...blank })}><Plus className="h-4 w-4" /> New Page</AdminButton>} /></div>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10 text-admuted">
              ...
              <th className="hidden p-3 text-left font-medium sm:table-cell">Updated</th>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="hidden p-3 font-mono text-xs text-brand lg:table-cell">/{p.slug}</td>
                  <td className="p-3"><LegacyPill tone={p.status === "published" ? "success" : "neutral"}>{p.status}</LegacyPill></td>
                  <td className="hidden p-3 text-slate-300 sm:table-cell">{p.updated_date ? new Date(p.updated_date).toLocaleDateString("en-US") : "-"}</td>
                  <td className="p-3"><div className="flex items-center justify-end gap-1.5">
                    <a href={`/${p.slug}`} target="_blank" rel="noopener noreferrer" className="rounded p-1.5 text-admuted hover:bg-white/10 hover:text-white"><ExternalLink className="h-4 w-4" /></a>
      ...
      <LegacyModal open={!!editing} onClose={() => setEditing(null)} title={editing?.id ? "Edit Page" : "New Page"} wide>
        {editing && (
          <div className="max-h-[70vh] space-y-4 overflow-y-auto pr-1">
            ...
            <Field label="Meta title"><AdminInput value={editing.meta_title} onChange={(e) => setEditing({ ...editing, meta_title: e.target.value })} /></Field>
      ...
      <LegacyModal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete page?">
