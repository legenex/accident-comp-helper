...
/* ==================================================================================
 * LEGACY COMPAT — exact old API, kept so pre-refactor pages (Pages, Blog,
 * Surveys, Landing Pages, Advertorials, ClaimBot, Experiments, Themes, and
 * the settings pages not yet migrated) keep working unchanged. New pages
 * should use the 22 primitives above, not these. Uses Tailwind's existing
 * navy/brand/admuted theme classes from tailwind.config.js, not `A` tokens —
 * that's the one place in this file a colour isn't sourced from `A`, and
 * it's intentional: it's the seam between the old and new design systems.
 * ================================================================================== */
export function Card({ className, children }) {
  return <div className={cn('rounded-xl border border-navyline bg-panel p-5', className)}>{children}</div>;
}
export function LegacyPill({ status, children, tone = 'neutral' }) {
  const tones = {
    success: 'bg-success/15 text-success', warning: 'bg-warning/15 text-warning',
    danger: 'bg-destructive/15 text-destructive', neutral: 'bg-white/10 text-admuted',
    blue: 'bg-brand/15 text-brand', purple: 'bg-purple-500/15 text-purple-300',
  };
  return (
    <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold', tones[tone])}>
      <span className={cn('h-1.5 w-1.5 rounded-full', {
        'bg-success': tone === 'success', 'bg-warning': tone === 'warning', 'bg-destructive': tone === 'danger',
        'bg-admuted': tone === 'neutral', 'bg-brand': tone === 'blue', 'bg-purple-400': tone === 'purple',
      })} />
      {children || status}
    </span>
  );
...
export function AdminButton({ children, variant = 'primary', className, ...props }) {
  const variants = {
    primary: 'bg-brand hover:bg-brand-hover text-white', secondary: 'bg-white/5 hover:bg-white/10 text-white border border-navyline',
    ghost: 'hover:bg-white/5 text-admuted hover:text-white', danger: 'bg-destructive/90 hover:bg-destructive text-white',
    purple: 'bg-purple-600 hover:bg-purple-700 text-white',
  };
  return <button className={cn('inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all', variants[variant], className)} {...props}>{children}</button>;
}
export function AdminInput({ className, ...props }) {
  return <input className={cn('w-full rounded-lg border border-navyline bg-navy/60 px-3 py-2 text-sm text-white placeholder-admuted/60 outline-none focus:border-brand focus:ring-1 focus:ring-brand/40', className)} {...props} />;
}
export function SearchBar({ value, onChange, placeholder = 'Search...' }) {
  return (
    <div className="flex flex-1 items-center gap-2 rounded-lg border border-navyline bg-navy/60 px-3 py-2 min-w-[180px]">
      <Search className="h-4 w-4 text-admuted" />
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="flex-1 bg-transparent text-sm text-white placeholder-admuted/60 outline-none" />
    </div>
  );
}
export function LegacySelect({ value, onChange, options, className }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className={cn('rounded-lg border border-navyline bg-navy/60 px-3 py-2 text-sm text-white outline-none focus:border-brand', className)}>
      {options.map((o) => <option key={o} value={o}>{o}</option>)}
    </select>
  );
}
export function LegacyEmptyState({ icon: Icon, title, body, action }) {
  return (
...
export function LegacyModal({ open, onClose, title, children, wide }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
